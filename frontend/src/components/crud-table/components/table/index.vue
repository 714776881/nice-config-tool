<template>
    <!--
        DataTable：表名，表格展示(table)，查询(search) ，批量导入，批量导出。

        DataRow：表主键，表单展示，编辑，增加，删除，保存。(editform)

        DataItem：字段名，数据展示，输入，默认值，候选项。(input)
    -->
            ...
            <template #bodyCell="{ column, record }">
                ...
                <template v-if="column.dataIndex.toLowerCase() === 'action'">
                    <a-space>
                        ...
                        <!-- 附加按钮 -->
                        <a-button style="margin-left: 6px;" v-for="(item, index) in props.config.addedButtons" :key="index" type="primary" ghost
                ...
                <template v-else>
                    ...
                    <!-- 候选项为一个 显示颜色标签 -->
                    <div v-else-if="column.options && column.options.length == 1">
                        {{ toLabel(record[column.dataIndex], column.options) }}
                    </div>
                    <!-- 显示文本 -->
                    <div v-else-if="record[column.dataIndex]">
                        {{ record[column.dataIndex].length > 20 ? record[column.dataIndex].substring(0, 20) + '...' : record[column.dataIndex] || "-"}}
                    </div>
                </template>
            </template>
        </a-table>
        <!-- 表单弹窗 -->
        <a-modal centered :width="props.config.columnConfig.length >= 8 ? 850 : 580" v-model:visible="isModalVisible"
...
const columns = computed<any[]>(() => {
    ...
    if (!columns.find(item => item.dataIndex.toLowerCase() == 'action')) {
        ...
        if (props.config.addedButtons && props.config.addedButtons.length > 0) {
            action.width = ((props.config.addedButtons.length + 2) * 60) + "px"
        }
        else {
            action.width = "160px";
        }
